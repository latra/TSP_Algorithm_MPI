...
bool StillRunning(tPila *pila, int slaveStatus[], int slaves) {
 //printf("QUedan %i elementos en la pila\n", pila->tope);
  if(PilaVacia(pila)) {
    for (int i = 0 ; i < slaves ; i++) {
      if (slaveStatus[i] == STATUS_WORKING)
        return true;
    }
    // Todos los esclavos desocupados y pila vacía
    return false;
  }
  return true;
}
...
void EscribeSolucion (tNodo *nodo,double t, int rango) 
{
     ...
     char rango_string[5];
     sprintf(rango_string, "_%d", rango);
     strcpy(output_file,input_file);
     strcat(output_file, rango_string);
